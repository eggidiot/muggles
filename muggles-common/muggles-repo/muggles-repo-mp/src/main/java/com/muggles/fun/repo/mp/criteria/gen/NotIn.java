 ...
 * 集合反向查询条件
 */
class NotIn implements IGenCriteria {
	/**
	 * 根据查询条件和MP对象生成MP查询条件
	 *
	...
	public QueryWrapper<?> translate(QueryWrapper<?> wrapper, MpCriteria criteria) {
		if (ObjectUtil.isNotNull(criteria.getValue())) {
			...
			wrapper.notIn(StrUtil.isNotBlank(criteria.getAttribute()) && CollectionUtil.isNotEmpty(array), StrUtil.toUnderlineCase(criteria.getAttribute()), array);
		}
		return wrapper;
	}
}
